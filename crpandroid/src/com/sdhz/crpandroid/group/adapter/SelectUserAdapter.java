...
public class SelectUserAdapter  extends BaseAdapter implements OnClickListener{
	...
	public SelectUserAdapter(List<UserInfo> itemsList, Context mcontext) {
		super();
	...
	private void sortitemsList(List  mitemList) {
		if(mitemList==null||mitemList.size()==0)
			return ;
		Collections.sort(mitemList, new Comparator<UserInfo>(){
			@Override
		...
		for (int i=0;i< mitemList.size();i++) {
			UserInfo  userInfo=(UserInfo) mitemList.get(i);
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(convertView==null){
			...
			convertView=LayoutInflater.from(mcontext).inflate(R.layout.select_person_activity_item, null);
